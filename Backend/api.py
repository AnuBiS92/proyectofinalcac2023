...
class Carta:
    ...
    def agregar_plato(self, nombre, descripcion, url, precio):
        self.cursor.execute("INSERT INTO carta (nombre, descripcion, url, precio) VALUES (?, ?, ?,?)",
                            (nombre, descripcion, url, precio))
    ...
    def modificar_plato(self, codigo, nuevo_nombre, nueva_descripcion, nueva_url, nuevo_precio):
        ...
        if plato:
            plato.modificar(nuevo_nombre, nueva_descripcion,
                            nueva_url, nuevo_precio)
    ...
    def listar_platos(self):
        ...
        for row in rows:
            codigo, nombre, descripcion, url, precio = row
            plato = {'codigo': codigo, 'nombre': nombre, 'descripcion': descripcion, 'URL': url, 'precio': precio}
            platos.append(plato)
...
class Pedido:
    ...
    def agregar(self, codigo, cantidad, carta):
        ...
        nuevo_item = Plato(codigo, plato.nombre, plato.descripcion, plato.url, plato.precio, cantidad)
    ...
    def mostrar(self):
        platos_pedido = []
        for item in self.items:
            plato = {'codigo': item.codigo, 'nombre': item.nombre, 'descripcion': item.descripcion, 'URL': item.url, 'cantidad': item.cantidad, 'precio': item.precio}
...
def agregar_plato():
    ...
    return carta.agregar_plato(nombre, descripcion, url, precio)
